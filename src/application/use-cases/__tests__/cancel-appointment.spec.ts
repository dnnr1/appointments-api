...
describe("CancelAppointmentUseCase", () => {
  it("cancels appointment", async () => {
    ...
    const result = await useCase.execute({ appointmentId: appointment.id.toString(), cancelLimitHours: 4 })
  ...
  it("rejects late cancel", async () => {
    const repo = new InMemoryAppointmentRepository()
    const cache = new InMemoryCacheProvider()
    const dateProvider = new FixedDateProvider(new Date("2026-01-10T15:00:00Z"))
    ...
    await expect(
      useCase.execute({ appointmentId: appointment.id.toString(), cancelLimitHours: 4 })
  ...
  it("rejects cancel if already canceled", async () => {
    ...
    const appointment = makeAppointment({ userId: "u1", serviceId: "s1", date: new Date("2026-01-10T16:00:00Z"), status: "canceled" })
    repo.items.push(appointment)
