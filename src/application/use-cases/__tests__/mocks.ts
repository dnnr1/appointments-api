...
export class InMemoryAppointmentRepository implements AppointmentRepository {
  ...
  async findByDate(date: Date): Promise<Appointment | null> {
    return this.items.find((item) => item.date.toDate().getTime() === date.getTime() && item.status === "scheduled") ?? null
  }
  ...
  async listByDateRange(start: Date, end: Date): Promise<Appointment[]> {
    return this.items.filter((item) => item.date.toDate() >= start && item.date.toDate() <= end)
  }
}
...
export const makeAppointment = (props: { userId: string; serviceId: string; date: Date; status?: "scheduled" | "canceled" }): Appointment => {
