...
import { InMemoryAppointmentRepository, InMemoryCacheProvider, FixedDateProvider, InMemoryUserRepository, InMemoryServiceRepository, makeAppointment } from "./mocks"

describe("CreateAppointmentUseCase", () => {
  it("creates appointment", async () => {
    const repo = new InMemoryAppointmentRepository()
    const cache = new InMemoryCacheProvider()
    const dateProvider = new FixedDateProvider(new Date("2026-01-10T10:00:00Z"))
    const users = new InMemoryUserRepository()
    const services = new InMemoryServiceRepository()
    users.add("u1")
    services.add("s1")

    const useCase = new CreateAppointmentUseCase(repo, users, services, dateProvider, cache)
  ...
  it("rejects schedule conflict", async () => {
    ...
    repo.items.push(makeAppointment({ userId: "u2", serviceId: "s1", date: new Date("2026-01-10T12:00:00Z") }))
  ...
  it("rejects user conflict", async () => {
    ...
    repo.items.push(makeAppointment({ userId: "u1", serviceId: "s1", date: new Date("2026-01-10T12:00:00Z") }))
